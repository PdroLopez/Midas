        ...
        <div class="card card-custom gutter-b">
            <div class="card-body pt-8 p-md-10 p-5">
                <span class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg text-hover-primary mb-1">
                    @if($solicitud->boleta_solicitud->first()->boleta->user->apellido != null)
                    {{ $solicitud->boleta_solicitud->first()->boleta->user->name }} {{ $solicitud->boleta_solicitud->first()->boleta->user->apellido }}
                    @else
                    {{ $solicitud->boleta_solicitud->first()->boleta->user->name }}
                    @endif
                </span>
                <br>

                <span class="text-dark-50 font-weight-normal font-size-md">{{ $solicitud->boleta_solicitud->first()->boleta->direccion->nombre }}</span>
                <span class="text-dark-50 font-weight-normal font-size-md">{{ $solicitud->boleta_solicitud->first()->boleta->direccion->comuna->nombre }}, {{ $solicitud->boleta_solicitud->first()->boleta->direccion->region->nombre }}</span>
                <div class="row mt-10">
                    @foreach($solicitud->imagen as $imagen)
                        <div class="col-md-4 col-6 mb-5">
                            <img src="{{ asset('public/img/solicitudes') }}/{{ $imagen->archivo }}" id="myImg" width="300" height="auto" class="img-thumbnail">
