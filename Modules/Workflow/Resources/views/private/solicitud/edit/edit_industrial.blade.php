					...
					<div class="card-body p-0">
						<div class="row justify-content-center py-8 px-8 py-lg-15 px-lg-10">
							<div class="col-xl-12 col-xxl-7">
								<form class="form mt-0 mt-lg-10 " id="kt_form" method="post" action="{{ asset('workflow/solicitud/editar-industrial') }}" file="true"
								  enctype="multipart/form-data">
									...
									<div class="pb-5" data-wizard-type="step-content" data-wizard-state="current">
										...
										<div class="form-group">
											...
											<select class="form-control select2" id="kt_select2_4" name="empresa" onchange="obtenerDirecciones(this.value)">
												...
												@foreach($empresas as $empre)
													@if(Session::has('edit_empresa_retiro_industrial'))
														@if($empre->id == Session::get('edit_empresa_retiro_industrial')->id)
														...
														@endif
													@else
														@if($empre->id == $boleta->empresas_id)
															<option value="{{$empre->id}}" selected>{{$empre->nombre}} - {{$empre->rut}}
																@if($empre->retc != null)
																- {{$empre->retc}}
																@endif
										...
										<div class="form-group" id="direcciones">
											@if(Session::has('edit_empresa_retiro_industrial'))
												<input type="hidden" id="empresa" value="{{Session::get('edit_empresa_retiro_industrial')->id}}">
												@if(Session::has('edit_direccion_retiro_industrial'))
													...
													<select class="form-control" name="direccion_usuario" id="direccion_usuario" onchange="cambiarSessionDir(this.value)">
														<option value="">Seleccionar</option>
														@foreach($direcciones_emp->where('empresas_id',Session::get('edit_empresa_retiro_industrial')->id) as $dir_emp)
															@if(Session::get('edit_direccion_retiro_industrial')->id == $dir_emp->id)
												...
												@endif
											@else
												<input type="hidden" id="empresa" value="{{$boleta->empresas_id}}">
												<label>Seleccionar Dirección</label>
												<select class="form-control" name="direccion_usuario" id="direccion_usuario" onchange="cambiarSessionDir(this.value)">
													<option value="">Seleccionar</option>
													@foreach($direcciones_emp->where('empresas_id',$boleta->empresas_id) as $dir_emp)
														@if($boleta->bk_direcciones_empresas_id == $dir_emp->id)
															<option value="{{$dir_emp->id}}" selected>{{$dir_emp->nombre}}</option>
														@else
															<option value="{{$dir_emp->id}}">{{$dir_emp->nombre}}</option>
														@endif
										...
										<div class="form-group">
											...
											<select class="form-control" name="tipo_servicio" id="tipo_ser">
												...
												@foreach($tipo_servicio as $tiposer)
													@if($tiposer->id == $boleta->tipo_servicio_id)
														<option value="{{$tiposer->id}}" selected>{{$tiposer->nombre}}</option>
													@else
										...
										<div class="form-group row">
											<div class="col-5">
												@if($boleta->destino == 0)
										  		...
										  		@else
										  			<input type="radio" id="destinomidas" name="destino" value="0"  onclick="destinoChange(this.value);">
											  		<label>Midas</label><br>
						            				<input type="radio" id="destinoterceros" name="destino" value="1" checked onclick="destinoChange(this.value);">
											  		<label>Terceros</label><br>
										  		@endif
											</div>
											...
											@if(Auth::user()->roles_id == 18)
												<select class="form-control" name="destino_midas" id="dest_midas">
													...
													@foreach($destinos as $dest)
														@if($boleta->destino_id == $dest->id)
															<option value="{{ $dest->id }}" selected>{{ $dest->nombre }}</option>
														@else
									...
									<div class="pb-5" data-wizard-type="step-content">
										...
										<table class="table" id="session_datos">
											...
											<tbody>
												@if(Session::has('edit_prod_industrial'))
													@foreach (Session::get('edit_prod_industrial') as $pro_ind)
														<tr>
													...
													@endforeach
												@else
													@foreach ($boleta->solicitudes as $pro_ind)
														<tr>
															<th>
																@if($pro_ind->solicitud->grupo != null)
																{{$pro_ind->solicitud->grupo->nombre}}
																@else
																 Sin Datos
																@endif
															</th>
															<th>
																@if($pro_ind->solicitud->clasificacion != null)
																{{$pro_ind->solicitud->clasificacion->nombre}}
																@else
																 Sin Datos
																@endif
															</th>
															<th>
																@if($pro_ind->solicitud->subcategoria != null)
																{{$pro_ind->solicitud->subcategoria->nombre}}
																@else
																 Sin Datos
																@endif
															</th>
															<th>
																@if($pro_ind->solicitud->tipo_producto != null)
																{{$pro_ind->solicitud->tipo_producto->nombre}}
																@else
																 {{$pro_ind->solicitud->otro_estado}}
																@endif
															</th>
															<th>
																{{$pro_ind->solicitud->peso}} Kg
															</th>
															<th>
																{{$pro_ind->solicitud->detalle_retiro}}
															</th>
														</tr>
													@endforeach
												@endif
										...
										<div class="form-group row">
											<div class="col-6">
												...
												@if($boleta->grua == 1)
								            		<input type="radio" id="gruaSi_id" onclick="confirmGrua(this.value);" name="grua" value="1" checked>
													<label>Si</label><br>
												@else
												...
												@endif
												@if($boleta->grua == 0)
								            		<input type="radio" id="gruaNo_id" onclick="confirmGrua(this.value);" name="grua" value="0" checked>
													<label>No</label><br>
												@else
												...
												@endif
							            		@if($boleta->grua == 2)
								            		<input type="radio" id="gruaNoAplica_id" onclick="confirmGrua(this.value);" name="grua" value="2" checked>
											...
											<div class="col-6">
							            		...
							            		<br>
							            		@if($boleta->encargado_grua != null)
								            		<input type="radio" id="encargadoSi_id" onclick="operarioGrua(this.value);" name="operario_carga" value="1" checked>
													<label>Si</label><br>
												@else
												...
												@endif
							            		@if($boleta->encargado_grua == null)
								            		<input type="radio" id="encargadoNo_id" onclick="operarioGrua(this.value);" name="operario_carga" value="0" checked>
													<label>No</label><br>
												@else
										...
										<div class="form-group row">
											<div class="col-5">
												...
												@if($boleta->estacion_camion == 1)
							            			<input type="radio" id="camionesSi_id" name="esta_camion" value="1" checked>
													<label>Si</label><br>
												@else
												...
												@if($boleta->estacion_camion == 0)
							            			<input type="radio" id="camionesNo_id" name="esta_camion" value="0" checked>
													<label>No</label><br>
												@else
													<input type="radio" id="camionesNo_id" name="esta_camion" value="0">
													<label>No</label><br>
												@endif
							            		@if($boleta->estacion_camion == 2)
							            			<input type="radio" id="camionesNoAplica_id" name="esta_camion" value="2" checked>
													<label>No Aplica</label><br>
												@else
											...
											@if($boleta->encargado_grua != null)
											<div class="col-7" id="div_encargado_grua" style="display: block;">
												<label>Nombre de Operario</label>
							            		<br>
												<input type="text" value="{{$boleta->encargado_grua}}" name="encargado_grua" class="form-control" id="encargado_grua_id">
											</div>
											@else
										...
										<div class="form-group row">
											<div class="col-4">
												...
												@if($boleta->solicitudes->first()->solicitud->accesos->imagen->count() != null)
								            		<input type="radio" id="adjuntoSi_id" name="adjunto_imagen" value="1" onclick="adjuntoImagen(this.value);" checked>
													<label>Si</label><br>
												...
												@else
													...
													<label>Si</label><br>
								            		<input type="radio" id="adjuntoNo_id"  name="adjunto_imagen" value="0" onclick="adjuntoImagen(this.value);" checked>
													<label>No</label><br>
												@endif
											</div>
											@if($boleta->solicitudes->first()->solicitud->accesos->imagen->count() != null)
												<div class="col-4" id="foto_adjunta_div" style="display: block;">
													<input type="file" name="accesos[]" multiple class="form-control">
												</div>
												<div class="col-4" id="foto_ya_adjunta" style="display: block;">
											...
											@else
												<div class="col-7" id="foto_adjunta_div" style="display: none;">
													<input type="file" name="accesos[]" multiple class="form-control">
												</div>
												<div class="col-1" id="foto_ya_adjunta" style="display: none;">
												</div>
											@endif
									...
									<div class="pb-5" data-wizard-type="step-content">
										...
										<div class="form-group row">
											<div class="col-6">
							            		...
							            		<input type="date" id="desdedate_id" class="form-control" value="{{$boleta->desde}}" name="desde_retiro">
											</div>
											<div class="col-6">
							           	 		<label>Hasta</label><br>
							            		<input type="date" class="form-control" id="hastadate_id" name="hasta_retiro" value="{{$boleta->hasta}}">
											</div>
										</div>
										<div class="form-group row">
											<div class="col-5">
											...
											<div class="col-7">
												@foreach($hr_dia as $hr)
													@if($boleta->horarios_dias_id == $hr->id)
									           	 		<input type="radio" name="jornada" value="{{$hr->id}}" checked>
												...
												@else
													<input type="radio" name="jornada" value="0" checked>
													<label>Por Definir</label>
												@endif
											</div>
										</div>
										...
										<div class="form-group row">
											...
											<div class="col-7">
							            		<input type="text" class="form-control" id="contactonombre_id" name="nombre_contacto" value="{{$boleta->nombre}}">
											</div>
										</div>
										<div class="form-group row">
											<div class="col-5">
											...
											<div class="col-7">
							            		<input type="number" class="form-control" id="contactotelefono_id" name="telefono_contacto" value="{{$boleta->telefono}}">
											</div>
										</div>
										<div class="form-group row">
											<div class="col-5">
											...
											<div class="col-7">
							            		<input type="email" class="form-control" id="contactoemail_id" name="email_contacto" value="{{$boleta->correo}}">
											</div>
										</div>
										<div class="form-group">
											<label>Obsevaciones(Opcional)</label>
											<textarea id="observaciones_id" class="form-control" name="observaciones">{{$boleta->observaciones}}</textarea>
										</div>
									</div>
...
@endsection
@include('workflow::private.solicitud.edit.productos_industrial')
@include('workflow::private.solicitud.edit.crear_empresa')
...
<div class="modal fade" id="agregar-direccion" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  	<div class="modal-dialog" role="document">
    	<div class="modal-content">
      		...
      		<form method="post" action="{{ asset('workflow/edit/agregar-nueva-direccion') }}">
				@csrf
				@if(Session::has('edit_empresa_retiro_industrial'))
      				<input type="hidden" id="empresa_id" name="empresa_id" value="{{Session::get('edit_empresa_retiro_industrial')->id}}">
      			@else
      				<input type="hidden" id="empresa_id" name="empresa_id" value="{{$boleta->empresas_id}}">
      			@endif
	...
	function operarioGrua(id){
	 ...
	 function adjuntoImagen(id){
	    if(id == 1){
	    	...
	    	document.getElementById('foto_ya_adjunta').style.display = "block";
	    }else{
	      	document.getElementById('foto_adjunta_div').style.display = "none";
	      	document.getElementById('foto_ya_adjunta').style.display = "none";
	    }
	 }
...
<script>
	...
	function obtenerDirecciones(id) {
		$("#empresa_id").val(id);
		$.get("{{ asset('/api/get-direccion/edit') }}/"+id, function(data, status){
